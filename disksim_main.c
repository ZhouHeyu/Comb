...
void warmFlash(char *tname){
  ...
  while(fgets(buffer, sizeof(buffer), fp)){
    ...
    delay = callFsim(MLC_blkno, mbcount, 0,1,0); 
    } 
  /*  if(MLC_blkno>=1048544) {
       MLC_blkno=MLC_blkno-1048544;
    }
    mbcount = ((blkno + bcount -1) / 4 - (blkno)/4 + 1) * 4;
    MLC_blkno /= 4;
    MLC_blkno *= 4;
    
    if(SLC_opagemap[MLC_blkno/4].ppn!=-1&&SLC_opagemap[MLC_blkno/4].free==0)
       delay = callFsim(MLC_blkno, mbcount, 0,0,1);
    else if(mbcount>8)
       delay = callFsim(MLC_blkno, mbcount, 0,0,2);
    else
       delay = callFsim(MLC_blkno, mbcount, 0,0,0);*/
