...
static void simpleflash_request_arrive (ioreq_event *curr)
{
  ...
  //flashsim
  accessTime = simpleflash_get_acctime(curr->devno, curr->blkno, curr->bcount, (curr->flags & READ),curr->flash_op_flag,curr->region_flag);//修改过
  delay2=0;   
...
double  simpleflash_get_acctime (int devno, unsigned int blkno, int bcount, int flags,int flash_op_flag,int region_flag)
{
  ...
  int flash_flag,SLC_blkno,MLC_blkno,a,b,c,d;
   ...
   if(flags==0){
      if(flash_op_flag==0){
         delay1 = callFsim(blkno, bcount,flags,0,0);
         delay=delay1+delay2;
      }else{
         delay = callFsim(blkno, bcount, flags,1,0);
     }
   }else{
        delay = callFsim(blkno, bcount, 1,1,0);
   }
